...
> Status note: this document still contains the detailed LangGraph design, but the current production path is the simplified FastAPI flow behind `/query` and `/query/approve`. LangGraph is retained mainly for legacy `/query/stream` and experiments.

## 1) Ключевые архитектурные решения

| # | Решение | Обоснование | Альтернативы и причины отказа |
|---|---------|-------------|-------------------------------|
| 1 | **Упрощённый FastAPI Text2SQL flow** для production `/query` + **LangGraph** для legacy `/query/stream` | Production path быстрее и проще в сопровождении: introspection -> history/memory -> LLM decision -> validate -> execute. LangGraph оставлен для экспериментов, стриминга и старого orchestration flow. | Только LangGraph для всего трафика (сложнее поддержка и latency), самописный orchestration layer (высокая стоимость поддержки) |
| 2 | **Qdrant** как векторное хранилище memory и metadata | Полноценный vector DB с rich filtering, payload-индексами и простым Docker runtime. В текущей реализации активно используется user memory слой; metadata indexing остаётся доступным для legacy/experimental flow. | ChromaDB (ограниченная фильтрация, single-node), Weaviate (тяжелее, избыточен для текущего объёма) |
...
| 7 | **Human-in-the-loop** для исполнения SQL | Любой SQL выполняется только после явного действия пользователя. В текущем UI это кнопка `Execute SQL`; API использует `/query/approve` с `approved=true`. | Автоматическое исполнение с whitelist (риск пропуска опасных запросов) |
...
4. Запускается Retriever.
   1. Для запроса строится embedding через OpenAI-compatible API.
...
1. **Indexing pipeline (offline)**
   ...
   - Построение embeddings через внешнюю embedding-модель.
   - Запись документов и payload-метаданных в Qdrant.

2. **Online retrieval pipeline**
   - Построение embedding пользовательского запроса той же внешней embedding-моделью.
...
| Параметр | Значение |
|----------|----------|
| Embedding model | Внешняя embedding-модель через OpenAI-compatible API (конфигурируется) |
...
| **Embedding API** | Построение embeddings для метаданных и запросов | HTTPS REST | Внешний API, зависит от сети и лимитов провайдера |
